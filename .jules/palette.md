...
## 2026-07-15 - Explicit Screen Reader Announcement for Message Authorship
**Learning:** In chat interfaces, visual alignment (left for user, right for system) is insufficient for screen readers. Without explicit context, messages are read aloud as continuous text, making it impossible to distinguish authorship.
**Action:** When creating message list items, always include explicit visually hidden text (e.g., `<span className="sr-only">User:</span>`) before the message content to ensure screen readers clearly announce who sent the message.
