...
## 2024-08-13 - [Journaling and Git Hygiene Constraints]
**Learning:** Overwriting `.jules/palette.md` using `>` in bash erases all previous critical learnings. Unintended lockfile generation (like `pnpm-lock.yaml`) pollutes the commit and violates size constraints. Routine work (like adding ARIA labels) shouldn't be journaled unless it reveals a unique project-specific pattern.
**Action:** Always use append mode (`>>`) with a preceding double newline for journal entries. Always verify `git status` and actively remove untracked generated lockfiles before submitting.
