...
## 2026-07-13 - Prevented O(N) Array Mapping Bottlenecks in Parent Components
**Learning:** Even if child components are memoized with `React.memo()`, a parent component mapping over a large array directly in its render function (e.g., `messages.map(...)` or `thoughts.map(...)`) will execute that O(N) mapping on every parent re-render. If the parent re-renders frequently due to unrelated state (like streaming telemetry or global connection status updates), this overhead becomes significant.
**Action:** Always wrap the array mapping logic in `useMemo` (e.g., `const renderedMessages = useMemo(() => messages.map(...), [messages])`) to prevent unnecessary O(N) recalculations on every unrelated parent render.
