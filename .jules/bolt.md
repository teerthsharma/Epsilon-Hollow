...
## 2026-07-19 - Prevent O(N) Render Mapping
**Learning:** Even if child components are memoized with `React.memo()`, a parent component mapping over a large array directly in its render function executes that O(N) mapping on every parent re-render. If the parent re-renders frequently due to unrelated state (e.g., streaming telemetry), the array mapping logic should be extracted and wrapped in a `useMemo` hook at the top level of the component.
**Action:** Use `useMemo` to wrap array mappings in parent components that render lists and have frequent, unrelated state updates.
