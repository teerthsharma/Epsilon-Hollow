...
## 2026-07-14 - Prevent O(N) array mapping costs in parent components
...
**Action:** Always wrap array mappings like `messages.map` inside `useMemo` in parent components that frequently re-render, especially in streaming applications like DSP Bus chat messages.
