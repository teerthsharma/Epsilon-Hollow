...
## 2024-07-07 - Prevent O(N) Typing Latency in React Chat Apps
**Learning:** Keeping input state (`input`, `setInput`) in the same top-level component that renders a large array of messages causes the entire component to re-render on every single keystroke, causing O(N) typing latency.
