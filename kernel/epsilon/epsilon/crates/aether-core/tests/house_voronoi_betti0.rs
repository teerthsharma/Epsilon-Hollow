...
fn betti_0_of_a_decomposition_of_a_connected_sphere_is_one() {
    let idx = SphericalVoronoiIndex::<4>::new([(0.5, 0.0), (1.0, 1.5), (2.0, 3.0), (2.5, 4.5)]);
    let b0 = idx.betti_0();
    println!(
        "K=4 well-separated centroids -> betti_0 = {b0}, capacity = {}",
        idx.capacity()
    );
    assert_eq!(
        b0, 1,
...
fn betti_0_does_not_grow_with_the_number_of_cells() {
    ...
    let b =
        SphericalVoronoiIndex::<4>::new([(0.5, 0.0), (1.0, 1.5), (2.0, 3.0), (2.5, 4.5)]).betti_0();
...
fn duplicate_centroids_leave_empty_cells_so_k_is_not_a_count_of_anything_real() {
    ...
    assert_eq!(
        idx.betti_0(),
        1,
        "beta_0 is still 1: the tiling still covers a connected S^2"
    );
}
