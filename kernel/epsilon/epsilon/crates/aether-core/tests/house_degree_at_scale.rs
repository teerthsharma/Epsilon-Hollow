...
struct R { max_e: usize, mean_e: f64, min_sep_ratio: f64, worst_reach: f64 }
...
fn measure(pts: &[ManifoldPoint<3>], eps: f64) -> R {
    ...
    for i in 0..n {
        for j in (i + 1)..n {
            ...
            if t[i] < t[j] { e[i].push(j); } else { e[j].push(i); }
        }
    }
    ...
    for (p, list) in e.iter().enumerate() {
        ...
        for &q in list {
            ...
            if r > worst_reach { worst_reach = r; }
        }
...
fn does_max_e_of_p_flatten() {
    ...
    println!("Sheehy 9.2: d(p,q) <= t_p for q in E(p), and E(p) separated by >= K*eps*(1-2eps)*t_p.");
    println!("At eps=1/3 that separation floor is K/9 ~ 0.11 for K ~ 1.");
    println!();
    println!("{:>6} {:>10} {:>10} {:>14} {:>14}", "n", "MAX E(p)", "mean E(p)", "max d/t_p", "min sep/t_p");
    let mut rows = Vec::new();
    for &n in &[256usize, 512, 1024, 2048, 4096, 8192] {
        let r = measure(&sphere(n), eps);
        println!("{n:>6} {:>10} {:>10.2} {:>14.4} {:>14.5}",
            r.max_e, r.mean_e, r.worst_reach, r.min_sep_ratio);
        rows.push((n as f64, r.max_e as f64));
    }
    println!();
    for w in rows.windows(2) {
        println!("  n {:>5.0} -> {:>5.0}: MAX E(p) x{:.3}",
            w[0].0, w[1].0, w[1].1 / w[0].1);
    }
    let first = rows[0].1;
    let last = rows[rows.len() - 1].1;
    println!();
    println!("MAX E(p) over the full 32x range in n: {first} -> {last}, x{:.3}", last / first);
    // Report, do not assert a verdict. The point of this iteration is the number.
    assert!(last >= first, "MAX E(p) fell, which would be a different finding entirely");
}
