...
fn sparse_adjacency<const D: usize>(pts: &[ManifoldPoint<D>], eps: f64)
...
fn count_triangles(adj: &[Vec<(usize, f64)>], t: &[f64]) -> usize {
    ...
    for (i, row) in adj.iter().enumerate() {
        for &(j, a) in row { entry[i * n + j] = a; entry[j * n + i] = a; }
    }
    let mut c = 0usize;
    for i in 0..n {
        for &(j, aij) in &adj[i] {
...
fn sphere_triangles_at_scale() {
    ...
    for &n in &[64usize, 128, 256, 512, 1024, 2048] {
        ...
        println!("{n:>6} {c:>12} {:>12.2} {:>12.2}", c as f64 / n as f64, e as f64 / n as f64);
        ...
        for i in 0..n {
            for j in (i + 1)..n {
                for k in (j + 1)..n {
                    ...
                    if e < t[i].min(t[j]).min(t[k]) { brute += 1; }
                }
            }
        }
