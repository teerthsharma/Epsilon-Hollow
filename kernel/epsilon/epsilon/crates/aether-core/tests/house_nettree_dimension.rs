...
fn dimension_profile<const D: usize>(pts: &[ManifoldPoint<D>], label: &str, expect: f64) {
    ...
    for l in 0..t.levels().saturating_sub(1) {
        ...
        println!(
            "  r {:.5}->{:.5}   N {:4}->{:4}   local dim = {:.3}",
            r0, r1, n0 as usize, n1 as usize, slope
        );
        slopes.push(slope);
    }
    if slopes.len() >= 3 {
        ...
        println!(
            "  ==> mid-scale mean dimension = {:.3}   (expected {expect})",
            mean
        );
    }
}
...
fn box_counting_dimension_from_the_net_tree() {
    // 1-D: circle in R^3
    let n = 2000;
    let circle: Vec<ManifoldPoint<3>> = (0..n)
        .map(|i| {
    ...
    let mut u = rng(12345);
    let sph: Vec<ManifoldPoint<3>> = (0..3000)
        .map(|_| {
            let z = 2.0 * u() - 1.0;
            let p = 2.0 * core::f64::consts::PI * u();
    ...
    // 3-D: uniform ball
    let mut u3 = rng(999);
    let ball: Vec<ManifoldPoint<3>> = (0..4000)
        .map(|_| loop {
            let (x, y, z) = (2.0 * u3() - 1.0, 2.0 * u3() - 1.0, 2.0 * u3() - 1.0);
            if x * x + y * y + z * z <= 1.0 {
                return ManifoldPoint::new([x, y, z]);
            }
        })
        .collect();
    ...
    let cantor: Vec<ManifoldPoint<3>> = pts
        .iter()
        .map(|p| ManifoldPoint::new([p[0], p[1], 0.0]))
        .collect();
