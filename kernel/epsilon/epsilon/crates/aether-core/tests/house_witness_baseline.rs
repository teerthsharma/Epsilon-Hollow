...
use aether_core::persistence::{persistent_homology, ComplexKind, PersistenceConfig, PersistenceDiagram};
...
fn cf(dim: usize, mp: usize, k: ComplexKind) -> PersistenceConfig {
    PersistenceConfig { max_homology_dim: dim, max_points: mp, max_simplices: 4_000_000,
        max_radius: f64::INFINITY, complex_kind: k }
}
fn circle(n: usize, r: f64) -> Vec<ManifoldPoint<2>> {
    (0..n).map(|i| { let t = 2.0*core::f64::consts::PI*(i as f64)/(n as f64);
        ManifoldPoint::new([r*t.cos(), r*t.sin()]) }).collect()
}
fn h1_pairs(d: &PersistenceDiagram) -> usize { d.pairs.iter().filter(|p| p.dimension==1).count() }
fn longest_h1(d: &PersistenceDiagram) -> (f64,f64) {
    d.pairs.iter().filter(|p| p.dimension==1)
...
fn witness_vs_exact_oracle() {
    ...
    for &n in &[16usize,24,32,40] {
        let t=Instant::now();
        let e = persistent_homology(&circle(n,1.0), cf(1,128,ComplexKind::VietorisRips)).unwrap();
        println!("n={:3}  total pairs={:6}  H1 pairs={:6}  build={:.3}s", n, e.pairs.len(), h1_pairs(&e), t.elapsed().as_secs_f64());
    }
    ...
    println!();
    println!("{:>9} {:>8} {:>10} {:>10} {:>12} {:>9} {:>8}", "landmark", "H1pairs", "pers", "death", "bottleneck", "time_s", "speedup");
    for &l in &[8usize,12,16,20,24,28,32] {
        ...
        match persistent_homology(&pts, cf(1,128,ComplexKind::Witness{max_landmarks:l})) {
            Ok(w) => {
                ...
                println!("{:>9} {:>8} {:>10.5} {:>10.5} {:>12.5} {:>9.4} {:>7.2}x",
                    l, h1_pairs(&w), wp, wd, b, el, t_exact/el.max(1e-9));
            }
