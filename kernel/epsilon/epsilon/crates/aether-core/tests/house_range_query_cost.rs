...
fn does_the_range_query_prune() {
    ...
    println!("{:>6} {:>12} {:>12} {:>10} {:>14}", "n", "scan (ms)", "query (ms)", "speedup", "hits agree");
    for &n in &[128usize, 256, 512, 1024, 2048] {
        ...
        for q in 0..n {
            scan_total += (0..n).filter(|&p| p != q && pts[q].distance(&pts[p]) <= t[q]).count();
        }
        ...
        for q in 0..n {
            q_total += tree.range_query(&pts, &cover, q, t[q]).len();
        }
        let q_ms = t1.elapsed().as_secs_f64() * 1e3;

        println!("{n:>6} {scan_ms:>12.2} {q_ms:>12.2} {:>10.2}x {:>14}",
            scan_ms / q_ms.max(1e-9), scan_total == q_total);
        assert_eq!(scan_total, q_total, "n={n}: hit counts differ, the query is not exact");
    }
}
