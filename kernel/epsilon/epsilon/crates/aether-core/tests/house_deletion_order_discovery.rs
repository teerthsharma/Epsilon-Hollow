...
fn quadratic(pts: &[ManifoldPoint<3>], t: &[f64], eps: f64) -> usize {
    ...
    for i in 0..n {
        for j in (i + 1)..n {
            ...
            let lim = t[i].min(t[j]);
            if d < lim && relaxed_entry_time(d, t[i], t[j], eps) < lim { c += 1; }
        }
    }
    c
}
...
fn deletion_ordered(pts: &[ManifoldPoint<3>], t: &[f64], eps: f64) -> usize {
    ...
    for &p in &order {
        for &q in &seen {
            ...
            if d < t[p] && relaxed_entry_time(d, t[p], t[q], eps) < t[p] { c += 1; }
        }
        seen.push(p);
    }
    c
...
fn deletion_ordered_discovery_matches_the_quadratic_scan() {
    let eps = 1.0 / 3.0;
    println!("{:>6} {:>12} {:>12} {:>10} {:>12}", "n", "quadratic", "ordered", "speedup", "counts agree");
    for &n in &[256usize, 512, 1024, 2048, 4096] {
        ...
        println!("{n:>6} {q_ms:>12.2} {o_ms:>12.2} {:>9.2}x {:>12}",
            q_ms / o_ms.max(1e-9), q == o);
        assert_eq!(q, o, "n={n}: deletion-ordered discovery found {o} edges, scan found {q}");
    }
}
