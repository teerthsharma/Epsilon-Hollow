...
fn reference_betti_0(data: &[u8], threshold: i16) -> u32 {
    ...
    for w in v.windows(2) {
        if (w[1] - w[0]).abs() > threshold { components += 1; }
    }
    components
}
...
fn uniform_data_is_one_component() {
    assert_eq!(compute_betti_0(&[0x90u8; 64]), 1, "64 identical bytes are one cluster");
}
...
fn beta_0_is_monotone_under_adding_a_duplicate_point() {
    ...
    assert_eq!(two, 1, "two coincident points are still one component, got {two}");
}

#[test]
fn separated_values_give_one_component_each() {
    assert_eq!(compute_betti_0(&[0u8, 100, 200]), 3, "three well-separated values");
}
...
fn matches_independent_reference_on_many_inputs() {
    ...
    for trial in 0..300 {
        ...
        assert_eq!(got, want, "trial {trial}: data={data:?} got={got} want={want}");
    }
}
...
fn beta_0_is_permutation_invariant() {
    ...
    for trial in 0..200 {
        ...
        assert_eq!(compute_betti_0(&data), compute_betti_0(&shuffled),
