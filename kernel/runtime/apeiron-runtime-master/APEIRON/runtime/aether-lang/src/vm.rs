    ...
    fn compile_stmt(&mut self, stmt: &Statement) {
        match &stmt.node {
            ...
            StmtKind::For(stmt) => {
                ...
                // 6. Increment Iterator: iter = iter + 1
                self.code.push(OpCode::LOAD(iter_idx));
                ...
                let loop_end_ip = self.code.len();
                let back_jump = (loop_start_ip as isize) - (loop_end_ip as isize) - 1;
                self.code.push(OpCode::JMP(back_jump));
            ...
            _ => {
                // TODO: Implement other control flows
            }
        }
    }
...
mod tests {
    use super::*;
    use crate::ast::{RenderStmt, RenderConfig, VarDecl, Span, Spanned};

    #[test]
    fn test_render_compilation() {
        // var x = 42;
        let var_decl = StmtKind::Var(VarDecl {
            type_hint: None,
            name: "x".to_string(),
            value: Spanned::new(ExprKind::Literal(Literal::Num(42.0)), Span::default()),
        });

        // render x;
        let render_stmt = StmtKind::Render(RenderStmt {
            target: "x".to_string(),
        ...
        let program = Program {
            statements: vec![
                Spanned::new(var_decl, Span::default()),
                Spanned::new(render_stmt, Span::default()),
            ],
        };
        ...
        if let OpCode::PUSH(v) = code[0] { assert_eq!(v, 42.0); } else { panic!("Expected PUSH"); }
        if let OpCode::STORE(idx) = code[1] { assert_eq!(idx, 0); } else { panic!("Expected STORE"); }
        if let OpCode::LOAD(idx) = code[2] { assert_eq!(idx, 0); } else { panic!("Expected LOAD"); }
        // PRINT and HALT
        match code[3] { OpCode::PRINT => {}, _ => panic!("Expected PRINT") }
        match code[4] { OpCode::HALT => {}, _ => panic!("Expected HALT") }
    ...
    #[test]
    fn test_for_loop() {
        use crate::ast::{ForStmt, Block, Range, Number, BinaryOp, ExprKind, Literal};

        // var sum = 0;
        let sum_decl = StmtKind::Var(VarDecl {
            type_hint: None,
            name: "sum".to_string(),
            value: Spanned::new(ExprKind::Literal(Literal::Num(0.0)), Span::default()),
        });

        // for i in 0:4 { sum = sum + i; }
        // Body: sum = sum + i
        let body_stmt = StmtKind::Var(VarDecl {
            type_hint: None,
            name: "sum".to_string(),
            value: Spanned::new(ExprKind::BinaryOp(
                Box::new(Spanned::new(ExprKind::Ident("sum".to_string()), Span::default())),
                BinaryOp::Add,
                Box::new(Spanned::new(ExprKind::Ident("i".to_string()), Span::default())),
            ), Span::default()),
        });

        let for_stmt = StmtKind::For(ForStmt {
            ...
            body: Block {
                statements: vec![Spanned::new(body_stmt, Span::default())],
            },
        });

        // sum;
        let sum_expr = StmtKind::Expr(Spanned::new(ExprKind::Ident("sum".to_string()), Span::default()));

        let program = Program {
            statements: vec![
                Spanned::new(sum_decl, Span::default()),
                Spanned::new(for_stmt, Span::default()),
                Spanned::new(sum_expr, Span::default()),
            ],
        };
        ...
        // Final result should be 6 (0+1+2+3)
