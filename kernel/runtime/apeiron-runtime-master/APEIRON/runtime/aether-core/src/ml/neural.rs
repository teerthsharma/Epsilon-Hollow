...
use std::vec::Vec;
#[cfg(not(feature = "std"))]
use alloc::vec::Vec; // Fixed: was alloc::vec
...
impl Activation {
    ...
    pub fn apply(&self, x: &Tensor) -> Tensor {
        match self {
            Activation::Softmax => {
                // Stable Softmax
                let max_val = x.max();
                let exponentials = x.map(|v| exp(v - max_val));
                let sum = exponentials.sum();
                exponentials.scale(1.0 / sum.max(1e-10))
            }
    ...
    pub fn derivative(&self, x: &Tensor) -> Tensor {
        match self {
            Activation::Softmax => Tensor::zeros(x.shape), // Handled specially usually, but placeholder
...
pub struct DenseLayer {
    ...
    pub biases: Tensor,  // [output_size, 1]
...
impl DenseLayer {
    ...
    pub fn backward(&mut self, grad_output: &Tensor, config: &OptimizerConfig) -> Tensor {
        ...
        // We lack outer product. But we can manually do it or use element-wise.
        // Actually: dW_ij = delta_i * input_j
        // This is MatMul: delta (Col) * input^T (Row) -> [Out, 1] * [1, In] -> [Out, In]
        
        // Ensure shapes are correct (n,1)
        // delta shape: (out, 1)
        // input shape: (in, 1) -> transpose -> (1, in)
        
        let input_t = last_input.transpose();
        let grad_w = delta.matmul(&input_t); 
