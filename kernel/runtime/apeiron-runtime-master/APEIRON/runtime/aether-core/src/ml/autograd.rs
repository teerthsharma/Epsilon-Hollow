...
impl<'a> Context<'a> {
    ...
    pub fn backward(&mut self, target: Variable) -> Vec<Option<Tensor>> {
        ...
        let mut grads: Vec<Option<Tensor>> = {
            ...
            { vec![None; max_idx] }
            #[cfg(not(feature = "std"))]
            { alloc::vec![None; max_idx] }
        };
        
        // Seed output gradient
        let target_tensor = self.heap.get(target.data).expect("Target lost");
        // Ones tensor
        let (r, c) = target_tensor.shape;
        let size = r * c;
        // Correct vector construction depending on cfg
        let ones_data = {
            #[cfg(feature = "std")]
            { vec![1.0; size] }
            #[cfg(not(feature = "std"))]
        ...
        grads[target.data.index] = Some(Tensor::new(ones_data, (r, c)));
        ...
        for op in self.tape.ops.iter().rev() {
            match op {
                ...
                Op::Mul { out, lhs, rhs } => {
                    let grad_out = grads[out.index].clone();
                    if let Some(grad) = grad_out {
                        let lhs_val: &Tensor = self.heap.get(*lhs).unwrap();
                        let rhs_val: &Tensor = self.heap.get(*rhs).unwrap();
                ...
                Op::MatMul { out, lhs, rhs } => {
                     ...
                     if let Some(grad) = grad_out {
                        ...
                        // dB = A^T @ dC
                        let transform_lhs = lhs_val.transpose();
                        let d_rhs: Tensor = transform_lhs.matmul(&grad);
                        Self::accumulate_grad(&mut grads, rhs.index, &d_rhs);
                     }
                },
                Op::ReLU { out, input } => {
                    let grad_out = grads[out.index].clone();
                    if let Some(grad) = grad_out {
                        let input_val: &Tensor = self.heap.get(*input).unwrap();
...
mod tests {
    ...
    // Helper to get value
    fn get_val(t: &Tensor, r: usize, c: usize) -> f64 {
        let (rows, cols) = t.shape;
        assert!(r < rows && c < cols);
        let data = t.get_snapshot();
        data[r * cols + c]
    }
    ...
    fn test_autograd_simple_mul() {
        ...
        let a = ctx.var(Tensor::new(vec![2.0], (1, 1)));
        let b = ctx.var(Tensor::new(vec![3.0], (1, 1)));
        let c = ctx.mul(a, b);
        ...
        assert_eq!(get_val(da, 0, 0), 3.0);
        assert_eq!(get_val(db, 0, 0), 2.0);
    }
    ...
    fn test_autograd_complex() {
        ...
        let x = ctx.var(Tensor::new(vec![2.0], (1, 1)));
        let x2 = ctx.mul(x, x);
        let y = ctx.add(x2, x);
    ...
    fn test_xor_backprop() {
        ...
        // Input: [1.0, 1.0] (1x2)
        let x = ctx.var(Tensor::new(vec![1.0, 1.0], (1, 2)));
        
        // W1: Identity [[1, 0], [0, 1]] (2x2)
        let w1 = ctx.var(Tensor::new(vec![1.0, 0.0, 0.0, 1.0], (2, 2)));
        
        // W2: [[1], [-2]] (2x1)
        let w2 = ctx.var(Tensor::new(vec![1.0, -2.0], (2, 1)));
        ...
        assert!((get_val(dy, 0, 0) - (-2.0)).abs() < 1e-6);
        
        let dw2 = grads[w2.data.index].as_ref().unwrap();
        // dL/dW2 = h1.T @ dy = [[1], [1]] @ [-2] = [[-2], [-2]]
        assert!((get_val(dw2, 0, 0) - (-2.0)).abs() < 1e-6);
        assert!((get_val(dw2, 1, 0) - (-2.0)).abs() < 1e-6);
    }
}
