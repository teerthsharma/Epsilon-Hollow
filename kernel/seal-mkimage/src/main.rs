...
fn check_tls_proof_text(text: &str) -> Result<(), String> {
    ...
    require_field_eq(line, "ecdhe=", "0", label)?;
    require_field_eq(line, "curve=", "x25519", label)?;
    require_field_eq(line, "psk_only=", "1", label)?;
    ...
    require_field_eq(line, "entropy=", "none", label)?;
    require_field_eq(line, "result=", "fail", label)?;
    Ok(())
}
