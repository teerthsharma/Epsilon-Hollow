    ...
    <div className="h-32 bg-gov-panel border-t border-gov-border flex flex-col shrink-0">
      <div className="h-6 flex items-center px-3 gap-2 text-[10px] uppercase tracking-wider text-gov-dim border-b border-gov-border">
        ...
        <button
          ...
          title="Clear logs"
          className="hover:text-gov-accent focus-visible:ring-2 focus-visible:ring-gov-accent focus-visible:outline-none rounded p-0.5"
        >
          <Trash2 size={10} />
        </button>
      </div>
      ...
      >
        {logs.length === 0 && (
          <div className="text-gov-dim/50 italic">Waiting for system telemetry...</div>
        )}
        {logs.map((log, i) => (
          <div
            key={i}
