...
impl AetherLinkKernel {
    ...
    /// Returns
    ///
    /// `(should_fetch, novelty_score)`
    /// - `should_fetch`: true if prefetch/bypass triggered (High Novelty)
    /// - `novelty_score`: Raw quantum probability [0.0, 1.0]
    #[inline]
    pub fn process_io_cycle(&mut self, lba_stream: &[u64]) -> (bool, f32) {
        self.cycles += 1;
...
mod tests {
    ...
    fn test_io_cycle() {
        ...
        let (decision, _score) = kernel.process_io_cycle(&stream);
        assert_eq!(kernel.cycles, 1);
        // Ensure decision is boolean
