...
impl BootInfo {
    ...
    fn raw(&self) -> Option<BootInformation> {
        unsafe { multiboot2::BootInformation::load(self.multiboot_start.0 as usize as *const multiboot2::BootInformationHeader).ok() }
    }
            ...
            if let Some(tag) = info.memory_map_tag() {
                for area in tag.memory_areas() {
                    f(MemoryRegion {
                        ...
                        kind: match area.typ().into() {
                             MemoryAreaType::Available => MemoryRegionKind::Usable,
                             MemoryAreaType::Reserved => MemoryRegionKind::Reserved,
                             MemoryAreaType::AcpiAvailable => MemoryRegionKind::Acpi,
                             MemoryAreaType::ReservedHibernate => MemoryRegionKind::Reserved,
    ...
    pub fn config_root(&self) -> Option<PhysAddr> {
        ...
        if let Some(tag) = info.rsdp_v2_tag() {
            ...
            return Some(PhysAddr(tag.signature().ok()?.as_ptr() as u64));
        }
