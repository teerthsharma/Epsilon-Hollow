...
impl<const D: usize> ManifoldPoint<D> {
    ...
    /// Optimization: Uses squared distance to avoid expensive sqrt() in inner loops
...
impl<const D: usize> SparseAttentionGraph<D> {
    ...
    pub fn geodesic_partition_centroid(&self, _target: ManifoldPoint<D>) -> Option<ManifoldPoint<D>> {
        if self.point_count == 0 {
            return None;
        }
