...
// OPTIMIZATION: Extract ThoughtItem into a memoized component.
// This prevents O(N^2) rendering bottlenecks by ensuring that when new chunks
// stream in, we only re-render the new items rather than the entire list.
const ThoughtItem = memo(function ThoughtItem({ text }: { text: string }) {
    return (
...
// OPTIMIZATION: Extract ChatMessage into a memoized component.
// Similarly prevents the entire message array from re-rendering every time a single
// new message or streamed chunk is added.
type Message = {
    id: string;
    sender: 'user' | 'apeiron';
    text: string;
