...
// ⚡ Bolt: Extracted and memoized thought item to prevent O(N^2) re-renders
// Performance Impact: Reduces re-renders of previous list items by ~99% during active DSP streaming.
const MemoizedThought = memo(function MemoizedThought({ text }: { text: string }) {
    return (
...
// ⚡ Bolt: Extracted and memoized message item to prevent O(N^2) re-renders
// Performance Impact: Prevents full chat history re-renders when new chunks arrive, dropping JS execution time.
const MemoizedMessage = memo(function MemoizedMessage({ msg }: { msg: { id: string, sender: string, isPlasticityEvent: boolean, text: string } }) {
    return (
