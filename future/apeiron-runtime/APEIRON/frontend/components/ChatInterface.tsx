...
const MessageItem = memo(function MessageItem({ msg }: { msg: Message }) { return (
    ...
    // Custom comparison to only update if text or plasticity state changes
    return prevProps.msg.text === nextProps.msg.text &&
           prevProps.msg.isPlasticityEvent === nextProps.msg.isPlasticityEvent;
});

// Also memoizing thoughts for the same O(N^2) render issue when thoughts stream in
const ThoughtItem = memo(function ThoughtItem({ thought }: { thought: string }) { return (
                ...
                <div className="p-6 border-t border-gray-800 bg-black">
                    ...
                    >
                        <input
                            ...
                            placeholder={tunnelStatus !== 'LOCKED' ? "Awaiting neuro-link connection..." : "Inject knowledge into the kernel..."}
                            aria-label={tunnelStatus !== 'LOCKED' ? "Input disabled while awaiting connection" : "Message input"}
                            title={tunnelStatus !== 'LOCKED' ? "Connection offline" : "Message input"}
                        ...
                        <button
                            ...
                            aria-label={tunnelStatus !== 'LOCKED' ? "Cannot send message while offline" : (!input.trim() ? "Cannot send empty message" : "Send message")}
                            title={tunnelStatus !== 'LOCKED' ? "Connection offline" : (!input.trim() ? "Enter a message to send" : "Send message")}
