...
type Message = { id: string; sender: 'user' | 'apeiron'; text: string; isPlasticityEvent: boolean; };

// React.memo prevents O(N^2) re-renders during streaming
const ThoughtMessage = React.memo(function ThoughtMessage({ thought }: { thought: string }) {
    return (
                ...
                <div className="space-y-6">
                    ...
                    <div className="p-4 rounded border border-gray-800 bg-gray-900/50 h-64 overflow-hidden flex flex-col">
                        ...
                        <div className="flex-1 overflow-y-auto font-mono text-xs space-y-2 text-gray-400 scrollbar-hide">
                            {thoughts.map((t, i) => (
                                <ThoughtMessage key={i} thought={t} />
                            ))}
                            <div ref={scrollRef} />
                        </div>
                    </div>
