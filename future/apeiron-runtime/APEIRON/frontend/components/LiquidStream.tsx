...
type Message = {
    ...
    timestamp?: string; // ⚡ Bolt: Capture timestamp once to prevent recalculation on render
};

// ⚡ Bolt: Performance optimization
// Wrapped LiquidStreamMessageItem in React.memo to prevent O(N^2) render performance issues.
// Especially critical during text streaming/updates where the state array updates cause the full list to re-render.
// React.memo ensures we only re-render the message that actually changed.
const LiquidStreamMessageItem = React.memo(function LiquidStreamMessageItem({ msg }: { msg: Message }) {
    return (
        <div
            className={cn(
...
export const LiquidStream = () => {
    ...
    return (
        <div className="h-full flex flex-col bg-black relative">
            ...
            >
                {messages.map((msg) => (
                    <LiquidStreamMessageItem key={msg.id} msg={msg} />
                ))}
