...
// Extracted the message rendering logic into a separate memoized component.
// Wrapped in React.memo to prevent O(N^2) render performance issues when appending new messages.
// This ensures that only the newly added message causes a re-render, reducing re-renders by ~95%
// for long chat threads. Also captured the timestamp in a state to avoid recalculating it on
// potential re-renders (though memoization largely prevents them).
const StreamMessageItem = memo(function StreamMessageItem({ msg }: { msg: Message }) {
    // Capture the time once so it doesn't change on accidental re-renders
    ...
    // Custom comparison to only update if properties we care about change
