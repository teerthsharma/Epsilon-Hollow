...
type Message = {
    ...
    timestamp?: string; // Memoized timestamp
};

// ⚡ Bolt: Performance optimization
// Extracting inline message rendering into a memoized component prevents O(N^2)
// full-list re-renders when the messages array updates (e.g. during streaming or frequent new messages).
// Storing the timestamp in the Message model ensures it is frozen upon creation.
    ...
    return (
        ...
        >
            <div className={cn(
                ...
                <span className="sr-only">{msg.role === 'user' ? 'User:' : 'Apeiron:'}</span>
                {msg.content}
...
export const LiquidStream = () => {
    ...
    // Hydration fix for initial state with timestamp
    useEffect(() => {
        setTimeout(() => {
            setMessages([{
                id: 1,
