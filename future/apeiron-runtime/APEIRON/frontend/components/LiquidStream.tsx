...
// ⚡ Bolt: Extracted and memoized individual message components.
// As the LiquidStream chat array grows, adding new messages previously
// caused the entire list to re-render. Memoization here (combined with
// freezing the timestamp via useState) drops the update time complexity
// from O(N^2) to O(1) for appending new items.
const StreamMessageItem = memo(function StreamMessageItem({ msg }: { msg: Message }) {
    // Freeze timestamp on creation so it doesn't recalculate if the component does happen to re-render
