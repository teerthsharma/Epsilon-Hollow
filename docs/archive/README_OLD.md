...
**Teleportation**: Moving a file between directories uses O(1) metadata surgery for directory/inode rewiring because the ManifoldPayload identity does not change. The persistent path now updates inode/directory metadata without rewriting raw file bytes; the boot marker requires `persistence_bytes_per_move=0`.
...
**Why metadata teleport?** A traditional cross-filesystem move copies data. ManifoldFS updates directory/inode topology and leaves the file's geometric identity stable. The current same-filesystem path updates metadata and leaves raw file bytes in place; cross-mount copy remains a separate fallback path.
...
| README/doc claim contract | △ | N/A | `seal-mkimage --check-doc-claim-contract .` enforces a limited allow/deny string contract for `README.md`, `docs/BENCHMARK_PLAN.md`, and `docs/CI.md`; it is a guardrail, not a complete proof that every public doc claim is backed by executable evidence |
...
| Bare-metal allocator benchmark marker | ✓ | △ | `seal-mkimage --check-benchmark-log ...\serial.log` requires `[BENCH] alloc-frame` with 64 successful alloc/free iterations, topological fast-path hits, zero bounded misses, no contiguous-probe drift, and no frame leak; `tools/ubuntu-alloc-bench`, `--check-ubuntu-benchmark-log`, `--compare-benchmark-logs`, and `--check-current-benchmark-proof` are the Ubuntu 26.04 proof gates, raw Ubuntu artifact pending; manual CI lane `ubuntu-alloc-baseline` requires a self-hosted native Ubuntu 26.04 runner |
...
| Scheduler select benchmark marker | ✓ | △ | `seal-mkimage --check-benchmark-log ...\serial.log` requires `[BENCH] scheduler-select-next`, gating the live `select_next_task` requeue marker across 64 iterations with ready count preserved, zero context switches, 8 Voronoi probes, max 9 bitmap tests, and max 256 priority-bucket scan; end-to-end wake latency remains a separate benchmark |
| TCP packet demux benchmark marker | ✓ | △ | `seal-mkimage --check-benchmark-log ...\serial.log` requires `[BENCH] tcp-packet-demux`, proving a listener-first same-port fixture routes payload bytes to the accepted socket; external packet latency remains a separate benchmark |
...
| Single-frame allocation O(1) | ✓ | △ | `--check-o1-allocator` plus boot log `[ALLOC] O(1) proof:` and `[BENCH] alloc-frame` gates; Ubuntu win requires `--check-current-benchmark-proof` against a current Seal proof manifest and same-machine Ubuntu 26.04 `tools/ubuntu-alloc-bench` artifact |
...
| Same-filesystem file move | ✓ | ✓ | Seal marker proves same-inode topology metadata surgery with `persistence_bytes_per_move=0`; Ubuntu rename is metadata-only on the same FS |
...
| **File move** | O(1) metadata surgery core; same-filesystem proof requires `persistence_bytes_per_move=0` | rename (O(1) same FS) | rename (O(1) same FS) | rename (O(1) same FS) | rename (O(1) same vol) | rename (O(1) same vol) |
| **Content-addressable lookup** | Bucketed geometric lookup (Voronoi cell narrows search; bucket cap pending) | No | No (needs `locate`) | No (needs `locate`) | No (Windows Search) | No (Spotlight) |
...
| **Topological data analysis** | Runtime-gated markers and selected Betti/Voronoi mechanisms | No | Userspace only | Userspace only | No | No |
| **Predictive prefetch** | T2 spectral contraction mechanism; benchmark pending | No | readahead heuristic | readahead heuristic | Superfetch/SysMain | Speculative prefetch |
