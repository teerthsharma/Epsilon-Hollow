...
| Oracle VirtualBox automated smoke | ✓ | ✓ | Fresh `smoke-vbox.ps1 -Seconds 240` rebuilds the VDI, then runs `--verify`, `--check-vbox-proof`, theorem/Aether/desktop/benchmark gates, and `--check-proof-manifest` against `vbox-smoke\proof-manifest.txt`; the manifest records proof VDI/EFI/log/screenshot fingerprints, `VBOX HARDDISK`, block device `0x800`, persistent ManifoldFS, desktop readiness, event loop, and current allocator markers |
